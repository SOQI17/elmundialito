...
export default function LiveMatchSimulator({
  ...
  // CONFIGURACIÓN DE MODO: 'simulated' (Simulación rápida local) o 'realtime' (Tiempo real compartido vía Firebase)
  const currentMode = match.mode || 'simulated';
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4 z-55 animate-fade-in animate-duration-150" id="live-simulator-modal-root">
      <div className="bg-slate-900 border border-slate-800 text-slate-100 rounded-3xl w-full max-w-3xl overflow-hidden flex flex-col h-[90vh] shadow-2xl relative">
        ...
        {/* PANEL: SELECCIÓN DE MODO DE JUEGO (HÍBRIDO SIMULACIÓN VS TIEMPO REAL) */}
        <div className="bg-slate-950 border-b border-slate-850 py-3 px-4 flex flex-col sm:flex-row justify-between items-center gap-3 shrink-0 select-none">
          <div className="flex items-center gap-1.5">
            <span className="text-xl">⚽</span>
            <div>
              <p className="text-[11px] font-black uppercase text-indigo-400 tracking-wider">Modo de Transmisión Activo</p>
              <p className="text-[9px] text-slate-400">Define si los resultados son ficticios (Simulación) o de fútbol real sincrónico.</p>
            </div>
          </div>
          ...
          <div className="flex bg-slate-900 p-1 rounded-xl border border-slate-800 self-stretch sm:self-auto">
            <button
              ...
              className={`flex-1 sm:flex-initial px-3.5 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1 cursor-pointer ${
                currentMode === 'simulated'
                  ? 'bg-indigo-650 bg-indigo-650 bg-indigo-600 text-white shadow-md'
            ...
            <button
              onClick={() => handleToggleMode('realtime')}
              className={`flex-1 sm:flex-initial px-3.5 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
                currentMode === 'realtime'
                  ? 'bg-emerald-600 text-white shadow-md border border-emerald-500 animate-pulse'
                  : 'text-slate-400 hover:text-white'
              }`}
            >
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping"></span>
              ⏱️ Fútbol Tiempo Real
            </button>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-r from-slate-950 via-slate-900 to-indigo-950 p-6 border-b border-slate-850 relative shrink-0">
          ...
          <div className="absolute top-4 left-6 flex items-center gap-1.5 bg-rose-600 font-extrabold text-[9px] uppercase text-white px-2.5 py-1 rounded-full tracking-widest animate-pulse">
            ...
            <span>{currentMode === 'realtime' ? 'SEÑAL REAL TIME' : 'SIMULACIÓN LOCAL'}</span>
          </div>
        ...
        {/* CONTROLES DINÁMICOS ACORDES AL MODO CONFIGURADO */}
        {currentMode === 'simulated' ? (
          /* CONTROLLER: SIMULACIÓN INTERACTIVA ACELERADA */
          <div className="bg-slate-950 border-b border-slate-800 p-4 shrink-0 flex flex-wrap gap-4 justify-between items-center text-xs">
            <div className="flex items-center gap-2">
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-white font-black text-xs transition-all cursor-pointer ${
                  isPlaying 
                    ? 'bg-amber-600 hover:bg-amber-500 shadow-md shadow-amber-900/10' 
              ...
              >
                {isPlaying ? (
                  ...
                  <>
                    ...
                    <span>{simGameMinute >= 90 ? 'REPETIR EN VIVO' : 'SIMULAR EN VIVO'}</span>
                  </>
                )}
              </button>
              ...
              {simGameMinute > 0 && (
                <button
                  onClick={resetSimSimulation}
                  id="btn-restart-simulation"
                  className="bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white p-2 rounded-xl transition-all font-bold flex items-center justify-center gap-1.5 cursor-pointer"
                  title="Reiniciar a Minuto 0"
                >
            ...
            <div className="flex items-center gap-1.5 bg-slate-900 p-1 rounded-xl border border-slate-800">
              <span className="text-[9px] uppercase font-bold text-slate-400 px-1.5">Frecuencia:</span>
            ...
            <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-xl">
              <label htmlFor="broadcast-toggle" className="font-black text-[9px] text-indigo-400 uppercase tracking-wider cursor-pointer flex items-center gap-1.5">
                <span className={`w-1.5 h-1.5 rounded-full bg-indigo-500 ${isBroadcastEnabled ? 'animate-ping' : ''}`}></span>
                💾 Auto-guardar a Firebase
              </label>
              <input
                type="checkbox"
                id="broadcast-toggle"
          ...
          <div className="bg-slate-950 border-b border-slate-850 p-4 shrink-0 flex flex-wrap gap-4 justify-between items-center text-xs">
            <div className="flex items-center gap-2.5">
              ...
              <p className="text-[10px] text-slate-450 text-slate-400 hidden sm:inline-block">
            ...
            <div className="flex items-center gap-2">
              {match.status === 'scheduled' || (match.status === 'live' && !match.liveStartTimestamp) ? (
                ...
                <div className="flex items-center gap-2">
                  <span className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 px-3 py-1 bg-stone-900 rounded-lg text-[10px] font-mono font-black uppercase">
                    ⚫ TRANSMITIENDO EN VIVID (Minuto {gameMinute}')
                  </span>
                </div>
              )}
            </div>
          ...
          <div className="w-full md:w-3/5 flex flex-col p-5 space-y-6 h-full overflow-y-auto min-h-[300px]" id="pitch-stats-view">
            ...
            <div className="bg-gradient-to-b from-emerald-800 to-emerald-950 h-36 rounded-2xl relative border border-slate-800 shadow-inner flex items-center justify-center overflow-hidden shrink-0">
              ...
              <div className="absolute top-2.5 right-3.5 bg-indigo-950/80 border border-indigo-500/30 text-indigo-300 font-mono text-[8px] px-2 py-0.5 rounded-md tracking-wider">
                {currentMode === 'realtime'
                  ? (match.status === 'live' ? '⚡ RELOJ REAL (1s = 1s)' : '⏸️ RETRANSMISIÓN')
                  : (isPlaying ? '🎰 SIMULADOR VELOZ' : '⏸️ JUEGO PAUSADO')}
              </div>
            </div>

            {/* LIVE CONSOLE BAR: AVAILABLE TO THOSE IN REALTIME MODE FOR DEMOING AND ADMIN */}
            {currentMode === 'realtime' && match.status === 'live' && (
