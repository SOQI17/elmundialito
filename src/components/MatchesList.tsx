...
export default function MatchesList({
  ...
  // Determinar si un partido está bloqueado para pronóstico (menos de 15 minutos para empezar, o ya iniciado, o terminado)
  const isMatchLocked = (match: Match): boolean => {
    ...
    // Hora actual fija de simulación o tiempo real (usando el actual 2026-05-27)
    const now = new Date('2026-05-27T13:48:26Z').getTime();

    // Bloquear si faltan menos de 15 minutos (15 * 60 * 1000 milisegundos)
    const fifteenMinutes = 15 * 60 * 1000;
    return (kickoff - now) < fifteenMinutes;
  };
  ...
  const getDayShortLabel = (dateStr: string) => {
    ...
    const dayNum = d.getUTCDate();
    return `${capitalizedW} ${dayNum}`;
  };

  // Get unique dates available in this specific phase
  const availableDatesInPhase = Array.from(
    new Set(
      matches
        .filter(m => m.phase === activePhase)
        .map(m => m.dateTime.split('T')[0])
    )
  ).sort();
  ...
  const getDateStats = (dateStr: string) => {
    ...
    const predictedCount = dayMatches.filter(m => forecasts.some(f => f.matchId === m.id && f.userId === currentUser.id)).length;
    return { totalCount, predictedCount };
  };

  // Obtener pronóstico del usuario actual para un partido
  const getMyForecast = (matchId: string): Forecast | undefined => {
    return forecasts.find(f => f.matchId === matchId && f.userId === currentUser.id);
  };

  // Guardar score de predicción en estado borrador local
  const handleScoreChange = (matchId: string, team: 'home' | 'away', currentVal: number, step: number) => {
    ...
    if (team === 'home') {
      homeS = Math.max(0, homeS + step);
    } else {
      awayS = Math.max(0, awayS + step);
    }

    setDrafts(prev => ({
      ...prev,
      [matchId]: { homeScore: homeS, awayScore: awayS }
    }));
  };

  // Confirmar y guardar el pronóstico (Apostar)
  ...
  const activeMatches = matches.filter(m => {
    ...
    if (activePhase === 'group' && activeGroupFilter !== 'all') {
      if (m.homeTeam.group !== activeGroupFilter) return false;
    }
    if (activeDateFilter !== 'all') {
      if (m.dateTime.split('T')[0] !== activeDateFilter) return false;
    }
    return true;
  });
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-6" id="matches-list-root">
      ...
      <div className="flex flex-col gap-4 border-t border-slate-100 pt-4" id="filters-container">
        {/* Row 1: Group filter (only during Group phase) */}
        {activePhase === 'group' && (
          <div className="flex flex-col gap-1.5" id="group-subfilter-container">
        ...
        {/* Row 2: Date prediction selector ("Apuestas por Fecha / Jornada") */}
        {availableDatesInPhase.length > 1 && (
          <div className="flex flex-col gap-1.5" id="date-subfilter-container">
      ...
      {activeMatches.length === 0 ? (
        <div className="text-center py-12 text-slate-400 text-sm">
          No hay partidos programados para esta fase aún.
        </div>
      ) : (
        <div className="space-y-8" id="matches-grid-view">
          {(() => {
            ...
            return sortedDateKeys.map((dateKey) => {
              ...
              const isTodaySimulation = dateKey === '2026-06-11'; // Opening Day highlight

              return (
                <div key={dateKey} className="space-y-4" id={`date-group-${dateKey}`}>
                  <div className="flex items-center gap-2.5 px-3 py-2 bg-slate-50 rounded-xl border border-slate-200/40">
                    ...
                    <h3 className="text-xs font-black text-slate-700 tracking-tight font-sans uppercase">
                    ...
                    {isTodaySimulation && (
                      <span className="ml-auto text-[9px] font-extrabold text-amber-750 text-amber-700 bg-amber-50 border border-amber-200 uppercase px-2 py-0.5 rounded-md animate-pulse">
            ...
            // Obtener predicciones de todos los demás amigos para este partido
              ...
              <div
                ...
                className={`border rounded-2xl transition-all overflow-hidden ${
                  locked 
                    ...
                    : 'border-slate-200 bg-white hover:border-indigo-150 hover:shadow-xs'
                }`}
              >
                {/* Top header of row */}
                <div className="px-4 py-3 bg-slate-50/40 border-b border-slate-100 flex flex-wrap items-center justify-between gap-2.5">
                  ...
                  <div className="flex items-center gap-2">
                    {isPlayed && (
                      <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 text-[9px] font-bold rounded-full uppercase">
                        Jugado
                      </span>
                    )}
                    {match.status === 'live' && (
                ...
                <div className="p-4 sm:p-6 grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
                  ...
                  {/* Left Column: Team names, flags, and real scores (if final string matches) */}
                  <div className="md:col-span-5 flex items-center justify-between gap-4">
                    {/* Home Team */}
                    <div className="flex flex-col items-center flex-1 text-center">
                      <span className="text-4xl filter drop-shadow-xs mb-1" role="img" aria-label={match.homeTeam.name}>
                        {match.homeTeam.flag}
                      </span>
                      <span className="text-xs font-bold text-slate-800">{match.homeTeam.name}</span>
                    </div>

                    {/* Real Match Score representation if played or live */}
                    ...
                    {/* Away Team */}
                    <div className="flex flex-col items-center flex-1 text-center">
                      <span className="text-4xl filter drop-shadow-xs mb-1" role="img" aria-label={match.awayTeam.name}>
                        {match.awayTeam.flag}
                      </span>
                      <span className="text-xs font-bold text-slate-800">{match.awayTeam.name}</span>
                    </div>
                  </div>

                  {/* Middle Column: Prediction Form for Active/Current simulation user profile */}
                  <div className="md:col-span-5 bg-slate-100/30 border border-slate-150 rounded-2xl p-4 flex flex-col items-center justify-center space-y-2">
                    ...
                    <div className="flex items-center gap-4">
                      {/* Prediction Team Home Input */}
                      <div className="flex items-center gap-1.5">
                        {!locked ? (
                          <button
                            id={`dec-forecast-home-${match.id}`}
                            onClick={() => handleScoreChange(match.id, 'home', userForecast?.homeScore || 0, -1)}
                            className="w-7 h-7 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 rounded-lg text-sm font-bold flex items-center justify-center shadow-xs cursor-pointer"
                          >
                            -
                          </button>
                        ) : null}
                      ...
                      {/* Prediction Team Away Input */}
                      <div className="flex items-center gap-1.5">
                        {!locked ? (
                          <button
                            id={`dec-forecast-away-${match.id}`}
                            onClick={() => handleScoreChange(match.id, 'away', userForecast?.awayScore || 0, -1)}
                            className="w-7 h-7 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 rounded-lg text-sm font-bold flex items-center justify-center shadow-xs cursor-pointer"
                          >
                            -
                          </button>
                    ...
                    {isPendingValue && !locked && (
                      <div className="flex flex-col items-center gap-1.5 w-full">
                        <span className="text-[10px] text-indigo-600 font-black animate-pulse uppercase tracking-wider">
                          ⚡ Cambios sin confirmar
                        </span>
                        <button
                          id={`btn-commit-box-${match.id}`}
                    ...
                    {/* Show calculated points badge if match is played */}
                    {isPlayed && userForecast && (
                      <div className="pt-2 border-t border-slate-100/50 w-full flex justify-center">
                        {(() => {
                          ...
                          return (
                            <span className={`px-2 py-0.5 text-[10px] font-bold rounded-full ${rating.style}`}>
                              {rating.label} (+{rating.points} Pts)
                            </span>
                          );
                        })()}
                      </div>
                  ...
                  {/* Right Column: Mini action or active save/bet button */}
                  <div className="md:col-span-2 flex flex-col gap-2.5 items-center md:items-end justify-center w-full">
                    
                    {/* Apostar Button - visible when there are pending draft changes */}
                    {isPendingValue && !locked ? (
                      <button
                        id={`btn-bet-action-${match.id}`}
                        onClick={() => handleCommitForecast(match.id)}
                        disabled={savingMatchIds[match.id]}
                        ...
                        title="Confirmar y Guardar Apuesta"
                      >
                        {savingMatchIds[match.id] ? (
                          <>
                          ...
                          <>
                            ...
                            <span className="tracking-tight uppercase">APOSTAR ⚽</span>
                          </>
                        )}
                      </button>
                    ) : null}

                    {/* All set state - visible when a forecast is saved and there's no active different draft */}
                    ...
                    {match.status !== 'scheduled' && (
                      <button
                        id={`btn-live-simulation-${match.id}`}
                        onClick={() => setSelectedLiveMatch(match)}
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-black transition-all grow md:grow-0 justify-center cursor-pointer w-full md:w-auto border ${
                          match.status === 'live'
                      ...
                      >
                        ...
                        <span>
                          {match.status === 'live' ? '🔴 En Vivo' : '📺 Ver Repetición'}
                        </span>
                      </button>
                    )}

                    {/* View Friends button and metadata */}
                    <button
                      id={`btn-toggle-others-${match.id}`}
                      onClick={() => setExpandedMatchId(expandedMatchId === match.id ? null : match.id)}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-semibold tracking-wide transition-all grow md:grow-0 justify-center cursor-pointer w-full md:w-auto"
                    >
                ...
                {/* Expanded estimation view: forecasts matrix of friends */}
                {expandedMatchId === match.id && (
                  <div className="bg-slate-50 p-4 border-t border-slate-150 text-xs space-y-2" id={`others-forecasts-panel-${match.id}`}>
                    ...
                    {otherForecasts.length === 0 ? (
                      <p className="text-slate-405 text-[11px] text-slate-400 italic">No hay más amigos que hayan pronosticado este partido aún.</p>
                    ) : (
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 pt-2">
                        {otherForecasts.map((f) => {
                          ...
                          // Seguridad: No revelar pronósticos de amigos si el partido aún no está bloqueado (evita copias)
...
// Auxiliar para conseguir badges visuales para los resultados calculados
function getScoreBadge(match: Match, forecast: Forecast) {
  ...
  if (match.homeScore === forecast.homeScore && match.awayScore === forecast.awayScore) {
    return {
      ...
      style: 'bg-emerald-100 text-emerald-800 border border-emerald-250',
  ...
  if (realWinner !== predWinner) {
    return {
      points: 0,
      label: 'Sin Puntos ❌',
      style: 'bg-slate-105 text-slate-500 bg-slate-100',
      badgeColor: 'bg-slate-500 text-white',
      reason: 'Diferente ganador'
    };
  }
  ...
  if (realWinner === 'Draw' && predWinner === 'Draw') {
    return {
      ...
      style: 'bg-indigo-100 text-indigo-800 border border-indigo-250',
      badgeColor: 'bg-indigo-500 text-white',
      reason: 'Mismo empate, goles dif.'
    };
  }

  // Trend + goal difference match (2 pts)
  if (realDiff === predDiff) {
    return {
      points: 2,
      label: 'Tendencia (Diferencia) 📈',
      style: 'bg-indigo-100 text-indigo-800 border border-indigo-255',
      badgeColor: 'bg-indigo-500 text-white',
      reason: 'Misma diferencia de goles'
    };
  }

  // Winner match only (1 pt)
  return {
    points: 1,
    label: 'Acierto Simple ⚽',
    style: 'bg-blue-100 text-blue-800 border border-blue-250',
